...
class SplunkEnv:
    ...
    def check_ready(self):
        ...
        self.wait_until_responsive(
            timeout=180.0,
            pause=0.5,
            check=lambda: self.is_responsive_splunk(),
        )
        LOGGER.info("Wait for remote side to be responsive hec")
        self.wait_until_responsive(
            timeout=30.0,
            pause=0.5,
            check=lambda: self.is_responsive_hec(),
        )
        ...
        result = self.search_util.checkQueryCountIsGreaterThanZero(
            search,
            interval=self.search_util.search_interval,
            retries=self.search_util.search_retry,
        )
    ...
    def is_responsive_hec(self):
        ...
        try:
            ...
            session_headers = {"Authorization": f"Splunk {self.hec_token}"}
    ...
    def send_hec_event(self, event_content):
        ...
        try:
            ...
            payload = {
                "sourcetype": "pytest-splunk-env:probe",
                "index": "_internal",
                "event": event_content,
            }
            ...
            response = requests.post(
                uri, verify=False, data=event[0], headers=session_headers
            )
            LOGGER.debug(f"Status code: {response.status_code}")
            if response.status_code in (200, 201):
                LOGGER.info("Splunk HEC is responsive.")
                ...
                LOGGER.error(
                    "Could not post to Splunk HEC. status code %s",
